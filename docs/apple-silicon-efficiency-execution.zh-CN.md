...
以下是源码核查与实施状态，不代表执行通过或物理收益。验证将在全部可实施工作完成后统一开始。

| 工作包 | 已有且复用 | 本次候选／缺口 | 状态 |
|---|---|---|---|
| E1 | packet/shadow 独占线程 scratch、generation-aware 动态 backing、上传去重、原生有序 multi-draw | encoder flush/end 失败仍释放；范围检查在 dedup 之前；OOM 明确失败；默认关闭的 partial upload 仅复制未覆盖区间；独立候选接线与计数 | 已实施，待集中验证 |
| E2 | 完整 SPIR-V/布局/编译选项 MSL key、PSO archive、加载 executor 编译、reload generation、同步 fallback | 长度前缀 key、缓存身份和内容摘要、并发独立临时文件；同步/异步 pending 单次消费与失效；预热队列有界且拒绝后按需创建 | 已实施，待集中验证 |
| E3 | generation-aware 附件 lifetime、deferred store、copy 去重、M3/M4 completion-owned allocator | 补 deferred store/fence 失败时的终结清理；保留已有同资源全 clear 的死 store 证明；不扩大不明消费者的消除范围 | 已实施与复用，待集中验证 |
| E4 | persistent terrain scene、GPU frustum/compact/ICB、publication guard、slice cache | 已发出或不确定发出 draw 时禁止重复 fallback；执行序号保护 cleanup 异常；显式 cadence 接入原 terrain 预算安全界限 | 已实施与复用，待集中验证 |
| E5 | 普通 SDL surface、frame receipts、Sodium pacing controller | 纯策略函数、前台/后台/最小化/空闲/可选电池限制、Dynamic FPS 委托、真实 limiter 接线；固定节奏功率导入、积分、ABBA 比较、合同/schema/测试/CLI | 已实施，待集中验证 |
| E6 | temporal/motion/coverage、source-pair receipts、独立 UI 合成、设备能力准入 | 后端 surface lease 单次消费、重配与隐藏/恢复时停旧 presenter、失效 history、frame epoch；复用现有 producer 准入 | 已实施与复用，待集中验证 |
...
- E4 的 GPU frustum/compact/ICB 路径已有准入；软件测试与 source mapping 不证明其在 Vanilla/Sodium/Iris 每个组合实际替代 CPU 工作。收敛任务负责 26.3 reference 合同，待合入其验证提交。
...
阶段 A 到此仅完成实现与人工源码审查，没有运行测试、编译或游戏。阶段 B 将先固定本地提交，取得收敛任务可同步的验证 SHA，再执行集中检查、修复失败并记录证据。
