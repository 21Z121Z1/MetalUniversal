 ...
 * The real render path batches admitted Java state through
 * {@code MetalRenderStatePacket}; on Metal 4 the native render bridge owns one
 * vertex, fragment, and compute {@code MTL4ArgumentTable} per in-flight slot.
 * This WeakHashMap/reflection snapshot duplicates bookkeeping and does not
 * replace any native binding.</p>
 *
 * <p>The mirror remains available behind the explicit
 * {@code metallum.iris.argumentSnapshotDiagnostics} switch for differential
 * diagnostics only. Its counters must never be accepted as performance-lane
 * activation evidence.</p>
 */
