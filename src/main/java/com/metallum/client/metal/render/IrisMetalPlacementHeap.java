...
/** Physical placement-heap allocation for proven non-overlapping colortex sides. */
final class IrisMetalPlacementHeap {
    ...
    static @Nullable Allocation tryCreate(
        ...
        if (!IrisMetalOptimizationPlan.ENABLE_HEAP_ALIASING
                || !"iris-colortex".equals(labelPrefix)
                || !MetalNativeBridge.irisPlacementHeapAvailable()) {
            return null;
        }
        IrisMetalTransientAllocationTelemetry.heapRequested();
        ...
        if (published == null || published.slotByResource().isEmpty()) {
            IrisMetalTransientAllocationTelemetry.heapRejected();
            return null;
        }
        ...
        for (Map.Entry<String, Integer> entry : published.slotByResource().entrySet()) {
            ...
            // The current receipt binds complete allocations through mip 0.
        ...
        try (Arena arena = Arena.ofConfined()) {
            ...
            for (int index = 0; index < requests.size(); index++) {
                ...
                records.set(ValueLayout.JAVA_LONG, base + 32L,
        ...
        try {
            ...
            IrisMetalTransientAllocationTelemetry.heapCreated();
