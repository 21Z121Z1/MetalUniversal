...
def evaluate(root: Path, head: str, jar_sha: str, dylib_sha: str) -> tuple[dict[str, Any], int]:
    ...
    for profile in PROFILES:
        ...
        mode = "vanilla" if profile == "V1" else "sodium" if profile == "S1" else "iris"
        if identity.get("renderer_mode") != mode:
            errors.append(f"{profile}: wrong renderer mode")
        if (identity.get("sodium_version") == "absent") != (mode == "vanilla"):
            errors.append(f"{profile}: wrong Sodium identity")
        identities[profile] = identity
        decisions[profile] = decision

    if len(identities) == len(PROFILES):
        reference = identities["V1"]
        for profile in ("S1", "I0", "I1"):
    ...
    result = {
        ...
        "errors": errors,
        "reason": (
            "V1, S1, I0 and I1 independently accepted the exact correctness-approved P1 production bits under matched physical Metal 4/residency trials"
            if not errors
...
def synthetic_identity(profile: str, head: str, jar: str, dylib: str) -> dict[str, Any]:
    identity: dict[str, Any] = {
        ...
        "sodium_version": "absent" if profile == "V1" else "test",
