...
case "$RENDERER" in vanilla|sodium|iris) ;; *) echo 'RENDERER must be vanilla, sodium, or iris' >&2; exit 2 ;; esac
...
run_lane() {
  ...
  set -e
  if [[ -d "$E2E_ROOT/build/evidence" ]]; then cp -R "$E2E_ROOT/build/evidence" "$lane_out/evidence"; fi
  if [[ -d "$E2E_ROOT/build/run/clientGameTest/logs" ]]; then cp -R "$E2E_ROOT/build/run/clientGameTest/logs" "$lane_out/logs"; fi
  printf '%s\n' "$status" > "$lane_out/exit-status.txt"
  (( status == 0 )) || return "$status"
  python3 scripts/agent/check_production_identity.py "$lane_out/evidence/artifact-identity.json" \
    --jar "$JAR" --source "$HEAD_SHA" --renderer "$RENDERER" --native-sha "$DYLIB_SHA" \
    --output "$lane_out/artifact-decision.json"
  ...
  # Evidence was preserved before checking the subprocess result.
}
...
python3 scripts/agent/check_metal4_main_e2e_pair.py \
  ...
  --output "$OUT/.pair-decision.pending.json"
...
python3 - "$OUT/.pair-decision.pending.json" "$RENDERER" <<'PY_ID'
import json, pathlib, sys
...
PY_ID
echo "P1 physical correctness pair: PASS ($RENDERER)"
echo "Evidence: $OUT"
