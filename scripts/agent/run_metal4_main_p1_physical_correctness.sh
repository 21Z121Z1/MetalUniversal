...
physical_preflight
RENDERER_MODE="${P1_RENDERER_MODE:-vanilla}"
case "$RENDERER_MODE" in vanilla|sodium|iris) ;; *) echo 'Invalid P1_RENDERER_MODE' >&2; exit 2;; esac

RUN_ROOT="${METALLUM_AGENT_RUN_ROOT:-$ROOT/build/agent-runs}"
STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
OUT="${METALLUM_P1_CORRECTNESS_OUT:-$RUN_ROOT/p1-metal4-main-correctness-$RENDERER_MODE-$STAMP}"
...
run_lane() {
  ...
  env -u METALLUM_HOSTED_METAL_OFFSCREEN \
    ...
    ./gradlew -p .github/ci/minecraft-e2e --no-daemon \
      "-PmetallumJar=$JAR" \
      "-Pmetallum.renderer=$RENDERER_MODE" \
      "-Pp1Metal4Lane=$lane" \
      ...
      2>&1 | tee "$lane_out/client.log" || {
        status=$?
        [[ ! -d "$E2E_ROOT/build/evidence" ]] || cp -R "$E2E_ROOT/build/evidence" "$lane_out/evidence-failed"
        return "$status"
      }

  python3 scripts/agent/verify_loaded_artifact.py \
    --identity "$E2E_ROOT/build/evidence/artifact-identity.json" \
