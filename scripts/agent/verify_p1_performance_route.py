...
PRODUCTION = (ROOT / ".github/ci/minecraft-e2e/build.gradle").read_text(encoding="utf-8")
require("runProductionPhysicalPerformance" in RUNNER,
        "physical performance still uses a development client")
require("'metallum.iris.semantic': (rendererProfile == 'iris').toString()" in PRODUCTION,
        "production launch does not independently gate Iris semantics")
require("check_production_identity.py" in RUNNER and "artifact-identity.json" in RUNNER,
        "physical performance does not verify actual loaded JAR/native identity")
require('P1_IRIS_CORRECTNESS_GATE' in MATRIX,
        "Iris performance lacks its own exact-runtime correctness gate")
