...
MANIFEST = ROOT / "validation/contracts/benchmark-profiles.json"
...
def main() -> None:
    ...
    if data.get("reference_branch") != "26.3-fabric-dev":
        fail("reference_branch must be 26.3-fabric-dev")
    ...
    if set(by_id) != {"V1", "S1", "I0", "I1"}:
        fail(f"profiles must be exactly V1/S1/I0/I1, got {sorted(by_id)}")
    ...
    for profile_id, profile in by_id.items():
        ...
        if stack.get("sodium") != (profile_id != "V1") or stack.get("iris_loaded") != (profile_id in ("I0", "I1")):
            fail(f"{profile_id} has the wrong optional renderer stack")
