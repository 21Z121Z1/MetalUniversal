...
if [[ "$PROFILE_ID" != V1 ]]; then RENDERER=iris; fi
case "$RENDERER" in vanilla|sodium|iris) ;; *) echo 'Invalid RENDERER' >&2; exit 2 ;; esac
...
read -r CORRECTNESS_JAR_SHA CORRECTNESS_DYLIB_SHA < <(
  python3 - "$CORRECTNESS_GATE" "$HEAD_SHA" "$RENDERER" <<'PY'
...
if identity.get("rendererMode") != sys.argv[3]:
    raise SystemExit("correctness gate renderer does not match the performance runtime")
...
./gradlew --no-daemon buildMacNative jar verifyProductionJarIsolation -x buildIOSNative -x buildIOSSpvc \
  2>&1 | tee "$OUT/build.log"
JAR="$(find "$ROOT/build/libs" -maxdepth 1 -type f -name '*.jar' ! -name '*-sources.jar' ! -name '*-dev.jar' ! -name '*-validation.jar' -print)"
[[ -f "$JAR" ]] || { echo 'Expected exactly one production JAR' >&2; exit 2; }
[[ "$(shasum -a 256 "$JAR" | awk '{print $1}')" == "$CORRECTNESS_JAR_SHA" ]] || { echo 'Production JAR differs from correctness' >&2; exit 3; }
[[ "$(shasum -a 256 src/main/resources/natives/macos/libmetallum.dylib | awk '{print $1}')" == "$CORRECTNESS_DYLIB_SHA" ]] || { echo 'Native differs from correctness' >&2; exit 3; }
...
run_trial() {
  ...
  {
    printf '[command] ./gradlew --no-daemon -p .github/ci/minecraft-e2e runProductionPhysicalPerformance -Pworld=%q' "$EVAL_WORLD"
    printf ' %q' "-PmetallumJar=$JAR" "-Pmetallum.renderer=$RENDERER" "-PmetallumSourceSha=$HEAD_SHA" "${args[@]}"
    printf '\n'
  } > "$trial_dir/command.txt"

  set +e
  MTL_DEBUG_LAYER=0 MTL_SHADER_VALIDATION=0 \
    ./gradlew --no-daemon -p .github/ci/minecraft-e2e runProductionPhysicalPerformance \
      "-Pworld=$EVAL_WORLD" "-PmetallumJar=$JAR" "-Pmetallum.renderer=$RENDERER" "-PmetallumSourceSha=$HEAD_SHA" "${args[@]}" 2>&1 | tee "$trial_dir/client.log"
  status=${PIPESTATUS[0]}
  set -e
  ...
  python3 scripts/agent/check_production_identity.py "$trial_dir/artifacts/validation/artifact-identity.json" \
    --jar "$JAR" --source "$HEAD_SHA" --renderer "$RENDERER" --native-sha "$CORRECTNESS_DYLIB_SHA" \
    --output "$trial_dir/artifact-decision.json"
