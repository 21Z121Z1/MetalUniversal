...
case "$PROFILE_ID" in
  ...
  I1)
    ...
    ;;
  *)
    echo "PROFILE_ID must be V1, S1, I0, or I1 (got $PROFILE_ID)" >&2
    exit 2
    ;;
esac

case "$PROFILE_ID" in
  V1) RENDERER_MODE=vanilla;;
  S1) RENDERER_MODE=sodium;;
  I0|I1) RENDERER_MODE=iris;;
esac
[[ "$WORLD" != */* && "$WORLD" != . && "$WORLD" != .. ]] || { echo 'WORLD must be a save directory name' >&2; exit 2; }
...
if [[ "$PROFILE_ID" == "I0" || "$PROFILE_ID" == "I1" ]]; then
...
read -r CORRECTNESS_JAR_SHA CORRECTNESS_DYLIB_SHA < <(
  python3 - "$CORRECTNESS_GATE" "$HEAD_SHA" "$RENDERER_MODE" <<'PY'
import json, pathlib, sys
path = pathlib.Path(sys.argv[1])
head = sys.argv[2]
data = json.loads(path.read_text(encoding="utf-8"))
if data.get("rendererMode") != sys.argv[3]:
    raise SystemExit("correctness gate belongs to a different renderer stack")
...
DYLIB="$ROOT/src/main/resources/natives/macos/libmetallum.dylib"
[[ "$(shasum -a 256 "$JAR" | awk '{print $1}')" == "$CORRECTNESS_JAR_SHA" ]] || { echo 'JAR differs from correctness gate' >&2; exit 3; }
[[ "$(shasum -a 256 "$DYLIB" | awk '{print $1}')" == "$CORRECTNESS_DYLIB_SHA" ]] || { echo 'Native differs from correctness gate' >&2; exit 3; }
...
if [[ "$PROFILE_ID" == "V1" || "$PROFILE_ID" == "S1" ]]; then
...
run_trial() {
  ...
  {
    printf '[command] ./gradlew --no-daemon -p .github/ci/minecraft-e2e runProductionClientValidation -Pworld=%q' "$EVAL_WORLD"
    printf ' %q' "${args[@]}"
    printf '\n'
  ...
  MTL_DEBUG_LAYER=0 MTL_SHADER_VALIDATION=0 \
    ./gradlew --no-daemon -p .github/ci/minecraft-e2e runProductionClientValidation \
      "-Pworld=$EVAL_WORLD" "-PclientRunDir=$ROOT/run" \
      "-PmetallumJar=$JAR" "-Pmetallum.renderer=$RENDERER_MODE" \
      "-Pp1Metal4Lane=$lane" "-Pp1SourceSha=$HEAD_SHA" \
      "-Pp1ProductionJarSha256=$CORRECTNESS_JAR_SHA" "-Pp1NativeDylibSha256=$CORRECTNESS_DYLIB_SHA" \
      "-PvalidationOutput=$trial_dir/artifacts/validation" "${args[@]}" 2>&1 | tee "$trial_dir/client.log"
  status=${PIPESTATUS[0]}
  set -e
  ...
  python3 scripts/agent/verify_loaded_artifact.py \
    --identity "$trial_dir/artifacts/validation/artifact-identity.json" \
    --mode "$RENDERER_MODE" --source "$HEAD_SHA" --jar "$JAR" --native "$DYLIB"
...
identity = {
    ...
    "sodium_version": "absent" if profile == "V1" else props.get("sodium_version", "unknown"),
    "renderer_mode": "vanilla" if profile == "V1" else "sodium" if profile == "S1" else "iris",
...
required_by_profile["S1"] = required_by_profile["V1"]
...
path.write_text(json.dumps({
    ...
    "benchmark_contract": "validation/contracts/benchmark-profiles.json",
    "identity": identity,
    "world": sys.argv[5],
    "pinned_runtime_config": {
        ...
        "shader_pack": None if profile in ("V1", "S1") else sys.argv[15],
        "metalfx_mode": "OFF",
