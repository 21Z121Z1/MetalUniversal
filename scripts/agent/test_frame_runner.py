...
from run_frame_workload import run_workload, workload_command
...
class FrameRunnerTest(unittest.TestCase):
    ...
    def test_missing_hardware_emits_unavailable_without_fabricating_an_exit_code_or_result(self):
        args = self.arguments()
        with patch.dict(os.environ, {}, clear=True), patch("run_frame_workload.subprocess.check_output", side_effect=["a" * 40, "b" * 40, ""]), \
                patch("run_frame_workload.physical_display", side_effect=RuntimeError("physical-validation-required: no real display")):
    ...
    def test_command_has_exact_binary_disposable_path_and_independent_metal_observer_modes(self):
        ...
        self.assertFalse(any(item.startswith("-PframeEvidenceWarmupSeconds=") for item in command))
